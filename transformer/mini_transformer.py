...
class MultiHeadAttention(torch.nn.Module):
    ...
    def self_attention(query, key, value, mask, dropout: torch.nn.Dropout):
        ...
        attention_score = attention_score.softmax(dim=-1) # (batch, head, seq_len, seq_len)
    ...
    def forward(self, q, k, v, mask):
        ...
        x, attention_score = MultiHeadAttention.self_attention(query, key, value, mask, self.dropout)
...
class EncoderBlock(torch.nn.Module):
    def __init__(self, self_attention: MultiHeadAttention, feed_forward: FeedForwardBlock, dropout:float ) -> None:
        ...
        self.residual_connection = torch.nn.ModuleList([ResidualConnection(dropout) for _ in range(2)])
...
class DecoderBlock(torch.nn.Module):
    def __init__(self, self_attention: MultiHeadAttention, cross_attention: MultiHeadAttention, feedforward: FeedForwardBlock, dropout:float) -> None:
        super().__init__()
        ...
        self.residual_connection = torch.nn.ModuleList([ResidualConnection(dropout) for _ in range(3)])
